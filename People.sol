...
contract People is Destroyable{
    ...
    mapping (address => Person) internal peopleMapping ;
    ...
    function createPerson (string memory name1, uint age1, uint height1) internal {
        ...
        if (peopleMapping[msg.sender].age != 0) {
            
            beforeUpdatedName1 = peopleMapping[msg.sender].name;
            beforeUpdatedAge1 = peopleMapping[msg.sender].age;
            beforeUpdatedHeight1 = peopleMapping[msg.sender].height;
            beforeUpdatedAlreadyTwentyOne1 = peopleMapping[msg.sender].alreadyTwentyOne;
            updating1 = true;
        }
        ...
        assert(
            keccak256(
                abi.encodePacked(
                    peopleMapping[msg.sender].name, 
                    peopleMapping[msg.sender].age, 
                    peopleMapping[msg.sender].height, 
                    peopleMapping[msg.sender].alreadyTwentyOne
                )
            ) 
    ...
    function insertPerson (Person memory newPerson2) private {
        ...
        peopleMapping[creator2] = newPerson2;
    }
